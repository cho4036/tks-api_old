...
type IUserRepository interface {
	...
	Create(accountId string, paasword string, name string) (domain.User, error)
...
type User struct {
	...
	Organization Organization `gorm:"many2many:organization_users"`
	Creator      uuid.UUID
	EmailAddress string
	Department   string
	Description  string
...
// Public members
func (r *UserRepository) GetUser(userId uuid.UUID) (user domain.User, err error) {
	return domain.User{}, fmt.Errorf("NOT IMPLEMENTED")
}

func (r *UserRepository) GetUserByAccountId(accountId string) (domain.User, error) {
...
func (r *UserRepository) Create(accountId string, password string, name string) (domain.User, error) {
	_, err := r.GetUserByAccountId(accountId)
	if err == nil {
		return domain.User{}, fmt.Errorf("Already existed user %s", accountId)
	}
...
func (r *UserRepository) AssignRole(accountId string, roleName string) error {
	user, err := r.getUserByAccountId(accountId)
	if err != nil {
...
// private members
func (r *UserRepository) getUserByAccountId(accountId string) (User, error) {
	user := User{}
